...
- [oh-my-codex](https://github.com/Yeachan-Heo/oh-my-codex) (OMX) 설치 (*선택* — Codex 기능 사용 시)
...
OMC/OMX를 감지하고, `.omi/` 상태를 초기화하고, 훅을 설정합니다.
...
npm으로 설치한 경우(`npm i -g oh-my-intelligence`) 터미널에서 다음 명령어도 사용 가능합니다:
...
> **참고:** 마켓플레이스로 설치한 경우 Claude Code 안에서 `/setup`과 `/route`를 사용하세요.
...
**OMI는 Codex 없이도 동작합니다.** OMX가 설치되지 않은 경우:
- 모든 Do 작업이 자동으로 Claude로 폴백
- Claude가 Do 최적화 프롬프팅(FallbackPromptSpec)을 수신
- 모든 기능이 작동 — Codex의 속도/비용 이점만 없을 뿐
- `omi doctor` 표시: "Claude fallback active for Do tasks"

이것은 의도된 설계입니다. Codex는 가속기이지, 필수 요소가 아닙니다.
